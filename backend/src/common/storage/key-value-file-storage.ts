...
export class KeyValueFileStorage<T> {
    private readonly logger: ContextLogger = new ContextLogger(KeyValueFileStorage.name);
    ...
    async onModuleInit(): Promise<void> {
        this.logger.log(ON_APP_INIT_CTX, `Initializing storage for table: ${this.tableName}`);
        const data = await this.storage.read();
        if (data) {
            this.data = data;
            const keyCount = Object.keys(this.data).length;
            this.logger.log(ON_APP_INIT_CTX, `Loaded ${keyCount} entries from ${this.tableName}`);
        } else {
            this.logger.log(ON_APP_INIT_CTX, `No existing data found for ${this.tableName}, starting with empty storage`);
        }
    }
    async set(ctx: Ctx, key: string, value: T): Promise<void> {
        ...
        this.logger.debug(ctx, `Successfully persisted key "${key}" in ${this.tableName}`);
    }

    async get(ctx: Ctx, key: string): Promise<T | undefined> {
        const exists = key in this.data;
        this.logger.debug(ctx, `Getting key "${key}" from ${this.tableName} - ${exists ? 'found' : 'not found'}`);
    ...
    async getMany(ctx: Ctx, keys: string[]): Promise<T[]> {
        ...
        this.logger.debug(ctx, `Getting ${count} entries from ${this.tableName} - ${count} entries found: ${foundKeys.join(', ')}`);
    ...
    async getAll(ctx: Ctx): Promise<T[]> {
        ...
        this.logger.debug(ctx, `Getting all entries from ${this.tableName} - ${count} entries found`);
    ...
    async delete(ctx: Ctx, key: string): Promise<void> {
        ...
        if (exists) {
            ...
            await this.storage.persist(this.data);
            this.logger.debug(ctx, `Successfully deleted key "${key}" from ${this.tableName}`);
        } else {
            this.logger.warn(ctx, `Attempted to delete non-existent key "${key}" from ${this.tableName}`);
        }
    }

    async deleteAll(ctx: Ctx): Promise<void> {
        const count = Object.keys(this.data).length;
        this.logger.warn(ctx, `Deleting all ${count} entries from ${this.tableName}`);
        this.data = {};
        await this.storage.persist(this.data);
        this.logger.log(ctx, `Successfully deleted all entries from ${this.tableName}`);
    }
