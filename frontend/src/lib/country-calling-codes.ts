export interface CountryCallingCode {
  flag: string;
  name: string;
  code: string; // digits only, e.g. '54'
}

/**
 * ITU-T E.164 country calling codes.
 * Argentina is listed first; the rest are sorted alphabetically.
 */
...
const ARGENTINA = COUNTRY_CODES[0];

/**
 * Find the best matching country for a stored E.164 phone number.
 * Tries 4, 3, 2, 1-digit codes in order (longest match wins).
 * Falls back to Argentina if no match found.
 */
export function countryFromPhone(phone: string): { country: CountryCallingCode; localNumber: string } {
  if (!phone) return { country: ARGENTINA, localNumber: '' };
  const digits = phone.replace(/\D/g, '');
  for (const len of [4, 3, 2, 1] as const) {
    const prefix = digits.slice(0, len);
    const match = COUNTRY_CODES.find(c => c.code === prefix);
    if (match) return { country: match, localNumber: digits.slice(len) };
  }
  return { country: ARGENTINA, localNumber: digits };
}
